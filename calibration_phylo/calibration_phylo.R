...
work.dir <-  "/home/dniyukuri/lustre/calibration_phylo"  # on CHPC
...
# df <- read.csv("/home/david/benchmark_master_model/calibration_phylo/results.benchmark.epi.mm.stats.csv")

df <- read.csv("/home/dniyukuri/lustre/calibration_phylo/results.benchmark.epi.mm.stats_seed_1.csv")
...
wrapper.simpact.default <- function(inputvector = inputvector){
  ...
  source("/home/dniyukuri/lustre/calibration_phylo/needed.functions.RSimpactHelp.R")
  source("/home/dniyukuri/lustre/calibration_phylo/complete.master.epidemic.metrics.R")
  
  # source("/home/david/benchmark_master_model/calibration_phylo/needed.functions.RSimpactHelp.R")
  # source("/home/david/benchmark_master_model/calibration_phylo/complete.master.epidemic.metrics.R")
...
wrapper.simpact4ABC.phylo_MCAR_100 <- function(inputvector = inputvector){
  ...
  source("/home/dniyukuri/lustre/calibration_phylo/needed.functions.RSimpactHelp.R")
  source("/home/dniyukuri/lustre/calibration_phylo/compute.summary.statistics.phylo.MCAR.R")
  ...
  simpact4ABC.phylo_MCAR_100 <- function(inputvector = inputvector){
    ...
    if (length(results) == 0){
      ...
      if (as.numeric(results["eventsexecuted"]) >= (as.numeric(cfg.list["population.maxevents"]) - 1)) {
        ...
        summ.stats <- compute.summary.statistics.phylo.MCAR(simpact.trans.net = simpact.trans.net,
  ...
  results.summ.stats <- tryCatch(simpact4ABC.phylo_MCAR_100(inputvector = inputvector),
...
model.sim <- wrapper.simpact4ABC.phylo_MCAR_100
sum_stat_obs <- mean.phylo_MCAR_100.features
...
stat.sim.phylo.cdf <- simpact.parallel(model = model.sim, 
                                       actual.input.matrix = par.sim,
                                       seed_count = seed.val,
                                       n_cluster = n_cores)


saveRDS(stat.sim.phylo.cdf, file = "raw_simulation_calibration_phylo.RDS")



# load("/home/david/benchmark_master_model/calibration_phylo/results/stat.sim_outputs.RData")


stat.sim <- stat.sim.phylo.cdf[,1:length(sum_stat_obs)] # results of (ii): summary statistics matrix obtained from simulations done with parameter matrix
...
abc_res_phylo <- abc::abc(target=sum_stat_obs, 
                          param=na.par.sim, 
                          sumstat=na.stat.sim, 
                          tol=alpha, 
...
saveRDS(abc_res_phylo, file = "abc_res_calibration_phylo.RDS")
...
stats.params.table_phylo <- stats.parameters(input = cal.val)



saveRDS(stats.params.table_phylo, file = "params.table_raw_simulation_calibration_phylo.RDS")
...
epi.metrics_params_phylo <- simpact.parallel(model = wrapper.simpact.default,
                                             actual.input.matrix = inputmatrix.params,
                                             seed_count = 1,
                                             n_cluster = 56)


saveRDS(epi.metrics_params_phylo, file = "epi.metrics_after_calibration_run_params_phylo.RDS")
...
saveRDS(epi.metrics_median_params, file = "epi.metrics_after_calibration_run_median_params_phylo.RDS")
