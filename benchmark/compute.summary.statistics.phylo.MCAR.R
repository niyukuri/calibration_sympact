...
compute.summary.statistics.phylo.MCAR <- function(simpact.trans.net = simpact.trans.net,
  ...
  if(length(mCAr.IDs)>10){
    ...
    mCAr.IDs.tree.calib <- phylogenetic.tree.fasttree.par(dir.tree = dirfasttree,
                                                          sub.dir.rename = sub.dir.rename,
                                                          fasttree.tool = "FastTreeMP", # "FastTree", # FastTreeMP
                                                          calendar.dates = "samplingtimes.all.csv",
                                                          simseqfile = paste0("cov.",seq.cov, ".mCAr.IDs.C.Epidemic.Fasta"),
                                                          count.start = 1977,
                                                          endsim = endpoint,
                                                          clust = TRUE) # FALSE) # TRUE
    
    
    
    tree.cal <- mCAr.IDs.tree.calib # phylo.tree # phylo.tree.keep
    ...
    colless.phylo_1 <- phyloTop::colless.phylo(tree.cal$intree, normalise = TRUE)
    
    sackin.phylo_1 <- phyloTop::sackin.phylo(tree.cal$intree, normalise = TRUE)
    
    
    branch_leng_summary_1 <- summary(tree.cal$edge.length) # summary(tree.cal) # https://rdrr.io/cran/ape/man/summary.phylo.html
    
    
    Depths_1 <- phyloTop::getDepths(tree.cal$intree) # depth of tips and nodes
    ...
    maxHeight_1 <- phyloTop::maxHeight(tree.cal$intree, normalise = TRUE)
